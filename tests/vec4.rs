...
fn test_add() {
    ...
    assert!(approx_eq(result.w, 12.0));
}
...
fn test_sub() {
    ...
    assert!(approx_eq(result.w, 3.0));
}

#[test]
fn test_mul_vec4() {
    let result = Vec4::new(2.0, 3.0, 4.0, 5.0) * Vec4::new(6.0, 7.0, 8.0, 9.0);
    assert!(approx_eq(result.x, 12.0));
    assert!(approx_eq(result.y, 21.0));
    assert!(approx_eq(result.z, 32.0));
    assert!(approx_eq(result.w, 45.0));
}
...
fn test_mul_scalar() {
    ...
    assert!(approx_eq(result.w, 10.0));
}
